...
#[test]
fn test_const_gets_added_to_scope() {
    tracing_subscribe();
    ...
    assert_that!(&variable.definition().kind()).is_equal_to(DefinitionKind::Const);
    assert_that!(&variable.name()).is_equal_to("FOO");
    assert_that!(&variable.definition().name().kind()).is_equal_to(Identifier);
    assert_that!(&variable.definition().node().kind()).is_equal_to(ConstItem);
